...
Dossier_medical::Dossier_medical(int num_doss,QString espece,QString maladie,QString nom_animal,QString race,QDate date_naiss,double poids,QString sexe,double temperature,QString sterilisation,QString etat_vaccin,QString traitement)
{this->num_doss=num_doss;this->espece=espece;this->maladie=maladie;this->nom_animal=nom_animal;this->race=race;this->date_naiss=date_naiss;this->poids=poids;this->sexe=sexe;this->temperature=temperature;this->sterilisation=sterilisation;this->etat_vaccin=etat_vaccin;this->traitement=traitement;}
...
bool Dossier_medical::ajouter()
{
    ...
    QString num_temp_string=QString::number(temperature);
         query.prepare("INSERT INTO DOSSIER_MEDICAL (NUM_DOSS,ESPECE,MALADIE,NOM_ANIMAL,RACE,DATE_NAISS,POIDS,SEXE, TEMPERATURE,STERILISATION,ETAT_VACCIN,TRAITEMENT) "
                       "VALUES (:NUM_DOSS,:ESPECE,:MALADIE,:NOM_ANIMAL,:RACE,:DATE_NAISS,:POIDS,:SEXE,:TEMPERATURE,:STERILISATION, :ETAT_VACCIN,:TRAITEMENT)");
...
{ QSqlQueryModel * model=new QSqlQueryModel();
 ...
 bool Dossier_medical::modifier(int num_doss,QString espece,QString maladie,QString nom_animal,QString race,QDate date_naiss,double poids,QString sexe,double temperature,QString sterilisation,QString etat_vaccin,QString traitement){
     QSqlQuery query;
     QString numD_string=QString::number(num_doss);
     QString num_poids_string=QString::number(poids);
     QString num_temp_string=QString::number(temperature);
        query.prepare(" UPDATE DOSSIER_MEDICAL set num_doss=:NUM_DOSS ,espece=:ESPECE, maladie=:MALADIE ,nom_animal=:NOM_ANIMAL, race=:RACE,"
                      " date_naiss=:DATE_NAISS,poids=:POIDS,sexe=:SEXE,temperature=:TEMPERATURE,sterilisation=:STERILISATION,etat_vaccin=:ETAT_VACCIN,traitement=:TRAITEMENT where num_doss=:NUM_DOSS");
        query.bindValue(":NUM_DOSS", numD_string);
        query.bindValue(":ESPECE", espece);
        query.bindValue(":MALADIE", maladie);
        query.bindValue(":NOM_ANIMAL",nom_animal);
